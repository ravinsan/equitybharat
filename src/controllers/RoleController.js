...
export const View = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ message: 'Role successfully retrieved.', role: role });
    } catch (error) {
        console.error('Error retrieving role:', error);
        return res.status(500).json({ message: 'An error occurred while retrieving the role.' });
    }
};
...
export const update = async (req, res) => {
    ...
    try {
           ...
           return res.status(200).json({message:"Role is successfully updated.", role:role});
    } catch (error) {
        return res.status(400).json({messagre:"Something went wrong!"});
    }
}
